...
public class WatchController {
    ...
    private String appleWatch = "applewatch";
    private String fitbit = "fitbit";
    ...
    public ModelAndView getUploadView(
            @ApiParam(value = "the watch type to be downloaded", required = true) @PathVariable("watchType") String watchType) {
        rollbar.info("In WatchController/getUploadView with api parameter: " +
                watchType);
        // getting the appropriate viewName
        String viewName = "";
        if (watchType.equals(appleWatch)) {
            viewName = "appleWatch";
        } else if (watchType.equals(fitbit)) {
            viewName = "fitbit";
        } else {
            return null;
        }
    ...
    @ApiImplicitParams({
            @ApiImplicitParam(name = "file", value = "the zipped file to be uploaded as multipart/form-data", type = "POST", required = true, paramType = "blob")
    })
    public ResponseEntity<JSONObject> upload(
            @ApiParam(value = "The watch type to be uploaded", required = true) @PathVariable("watchType") String watchType,
    ...
    public ResponseEntity<JSONObject> processFiles(
            @ApiParam(value = "id of the file to be processed, which is the raw data id", required = true) @PathVariable("id") String rawDataId,
            @ApiParam(value = "The watch type to be processed", required = true) @PathVariable("watchType") String watchType) {
    ...
    public ResponseEntity<JSONObject> predictWatch(
            @ApiParam(value = "id of the file to be predicted, which is the processed data id", required = true) @PathVariable("id") String processedDataId,
            @ApiParam(value = "the machine learning model: svm, randomForest, rotationForest, decissionTree", required = true) @PathVariable("predictionmodel") String predictionModel,
            @ApiParam(value = "the watch type to be predicted", required = true) @PathVariable("watchType") String watchType) {
    ...
    public ResponseEntity<JSONObject> downloadFile(HttpServletRequest req, HttpServletResponse resp,
            @ApiParam(value = "id of the file to be downloaded", required = true) @PathVariable("id") String id,
            @ApiParam(value = "type of file to be downloaded: process or predict", required = true) @PathVariable("type") String type,
            @ApiParam(value = "the watch type to be downloaded", required = true) @PathVariable("watchType") String watchType) {
    ...
    public ResponseEntity<JSONObject> list(HttpServletRequest request,
            @ApiParam(value = "type of the apple watch data to be listed: raw, processed, predicted", required = true) @PathVariable("type") String type,
            @ApiParam(value = "the watch type to be downloaded", required = true) @PathVariable("watchType") String watchType) {

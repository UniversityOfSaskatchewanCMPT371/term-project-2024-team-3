...
test("Processed Files Page Test", async ({ page }) => {
    ...
    await expect(page.locator("#root")).toContainText(
        `Step 2 - Data prediction and download: In this step you can either 
      download the.csvof your Apple Watch or Fitbit data or, use our machine 
      learning methods to predict lying, sitting, and walking at difference 
      ...
      click the Predict File button. Once prediction is complete, move to the 
      predicted files page.`,
    );
    await expect(page.getByText("SVMRandom ForestDecission Tree")).toBeVisible();
    await page.getByRole("button", { name: "Predict File" }).click();
    await page.getByRole("button", { name: "Download File" }).click();
    await page.getByText("Go To Predicted Files").click();
    await page.waitForURL("**/PredictedDataPage");
